...
- 창의 X 를 누르면 앱은 **시스템 트레이로 숨고 백그라운드에서 계속 실행**된다. 다시 열려면 트레이 아이콘을 클릭한다.
- 앱이 실행 중일 때 다시 실행하면 새 창을 띄우지 않고 **기존 창을 앞으로 가져온다** (단일 인스턴스).
- **완전 종료**는 트레이 아이콘 우클릭 → **종료**. 이때 앱이 띄운 `ollama serve` 와 모델을 로드한 자식(runner) 프로세스를 트리째 정리해 메모리를 해제한다.
- 앱이 실행되기 전에 이미 사용자가 직접 `ollama` 를 띄워 뒀다면(경우 1) 종료 시 그 인스턴스는 건드리지 않는다.
